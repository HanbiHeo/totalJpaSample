...
public interface MemberRepository  extends JpaRepository<Member, Long> {
    // JpaRepository를 상속받아 이미 구현된 메서드를 사용할 수 있음
    // findByEmail: 이메일을 기준으로 멤버 객체를 찾는 메서드
    // Optional은 값이 없을 수 있는 경우 사용되며, null을 허용하지 않음을 의미
    //fintBy 해당하는 멤버 객체를 찾아달라는 쿼리문(select문)을 만듦. 완벽한 카멜 표기법으로 만들어야한다.
    //Optional = null을 허용하지 않겠다는 뜻.
