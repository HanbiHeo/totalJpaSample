...
import com.kh.totalJpaSample.dto.BoardDto;
...
import com.kh.totalJpaSample.repository.BoardRepository;
import com.kh.totalJpaSample.repository.CategoryRepository;
    ...
    public boolean saveBoard(BoardDto boardDto) {
        try {
            ...
            Member member = memberRepository.findByEmail(boardDto.getEmail()).orElseThrow(
                    () -> new RuntimeException("해당하는 회원이 존재하지 않습니다.")
            );
            Category category = categoryRepository.findById(boardDto.getCategoryId()).orElseThrow(
                    () -> new RuntimeException("해당하는 카테고리가 존재하지 않습니다.")
            );
